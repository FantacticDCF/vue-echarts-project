...
var $emit = function $emit(EeventName, params) {
  if (!EventList[EeventName]) return;
  var EventLists = EventList[EeventName];
  EventLists.map(function (cb) {
    params ? cb(params) : cb();
  });
};
...
var $off = function $off(EventName, callback) {
  if (EventList[EventName]) {
    var EventListsOff = EventList[EventName];

    if (callback) {
      EventList[EventName] = EventListsOff.filter(function (cb) {
        return cb != callback;
      });
    } else {
